...
const APPLICATION_TITLES: Record<string, string> = {
  ...
  // Headline is 77 characters with the " | YILON" suffix — over the SERP budget.
  'inflatable-boats': 'Inflatable Boat & SUP Zippers',
};
...
/**
 * The single share card, generated by scripts/build-social.mjs.
 *
 * Exported because the three route templates that build their own openGraph
 * object (guides, applications, blog posts) still have to name it explicitly —
 * Next replaces the whole object rather than merging the layout's.
 */
 ...
 *
 * Next REPLACES the `openGraph` object rather than deep-merging it with the
 * layout's, so a page that omits this inherits the layout's values wholesale.
 * That is how 27 pages came to advertise the homepage as their `og:url` and to
 * share one generic og:title — including product pages, whose card said nothing
 * about the product.
 *
 * Every field is derived from the page's own metadata so the two cannot drift:
 * og:title mirrors `<title>` and og:description mirrors the meta description.
 *
 * @param path Route path with a leading slash. Use `/` for the homepage — that
 *   resolves to the bare origin rather than `https://example.com/`.
 */
